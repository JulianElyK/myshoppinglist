...
@section('content')
  <div class="my-3 w-4/5 h-5/6 flex flex-col">
    <div class="m-3 p-4 w-full h-full bg-white rounded-xl">
      <form action="/shoppinglist/add" method="post" class="w-full h-full ">  
        @csrf
        ...
        <div class="my-1 flex justify-between">
          ...
          <input type="datetime-local" name="datetime" id="datetime" placeholder="Y-m-d\TH:i:s" value="{{ (new \DateTime())->format('Y-m-d\TH:i:s') }}" required
          class="bg-gray-100 border-2 w-5/6 p-2 my-2">
        </div>
        ...
        <div class="mt-4 mb-1 h-2/5 flex justify-between">
          <div class=" w-full flex-col">
            ...
            <div id="ShoppingItems" class="flex-row overflow-auto max-h-40">
              <div id="row-1" class="flex ShoppingItem">
                <div class="w-1/12 border-2"><input type="text" class="pl-2 w-full" name="numbers[]" class="w-full" disabled value="1"></div>
                <div class="w-3/12 border-2"><input type="text" class="pl-2 w-full" name="itemnames[]" class="w-full" value="{{ old('itemname.0') }}" required></div>
                <div class="w-1/12 border-2"><input type="text" class="pl-2 w-full" name="amounts[]" class="w-full" value="{{ old('amount.0') }}" required></div>
                <div class="w-2/12 border-2"><input type="text" class="pl-2 w-full" name="units[]" class="w-full" value="{{ old('unit.0') }}" required></div>
                <div class="w-5/12 border-2"><input type="text" class="pl-2 w-full" name="memos[]" class="w-full" value="{{ old('memo.0') }}"></div>
  ...
  <script>
    ...
    function AddRow() {
      ...
      col_itemname.innerHTML = '<input type="text" class="pl-2 w-full" name="itemnames[]" class="w-full" value="{{ old("itemname.' + (row_count - 1) +'") }}" required>';
      col_amount.innerHTML = '<input type="text" class="pl-2 w-full" name="amounts[]" class="w-full" value="{{ old("amount.' + (row_count - 1) +'") }}" required>';
      col_unit.innerHTML = '<input type="text" class="pl-2 w-full" name="units[]" class="w-full" value="{{ old("unit.' + (row_count - 1) +'") }}" required>';
      col_memo.innerHTML = '<input type="text" class="pl-2 w-full" name="memos[]" class="w-full" value="{{ old("memo.' + (row_count - 1) +'") }}">';
